"""
Системные настройки разработчика, палитры цветов и сохранение пользовательского вида отображения
"""
import os
import json
...
P2_PALETTE = [
    ...
    "#00FFCC", "#FF6F00", "#AA00FF", "#00E5FF", "#FF3366"
]

SOUND_SENSOR_COLOR = "#FF5500"
...
def determine_default_axis(label_name: str) -> str:
    """Определяет сторону оси по умолчанию: C и RPM -> left, остальные -> right"""
    l = label_name.lower()
...
def load_sensor_profile(mode: str = None, config_path: str = DEFAULT_CONFIG_PATH) -> dict:
    """Загружает sensors_config.json с чистыми дефолтами (все скрыто, все оси слева)"""
    ...
    target_mode = (mode or data.get("active_mode", "CPU")).upper()
    ...
    # 1. Панель P1 (все оси слева, все скрыты по умолчанию)
    ...
    # 2. Панель P2 (все оси слева, все скрыты по умолчанию)
    ...
    # Микрофон в P2 (скрыт по умолчанию, ось слева)
    ...
    return {
        ...
        "summary_dir_name": mode_data.get("summary_dir_name", target_mode.lower()),
        "chart_title_prefix": mode_data.get("chart_title_prefix", f"{target_mode} Thermal & Electrical Load Dynamics"),
...
def save_user_view_state(mode: str, state_dict: dict, path: str = VIEW_STATE_PATH):
    """Сохраняет текущий вид (видимость, стороны осей, активность M) в view_state.json"""
    all_states = {}
...
def load_user_view_state(mode: str, path: str = VIEW_STATE_PATH) -> dict:
    """Загружает сохраненный вид для режима (CPU/GPU)"""
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                all_states = json.load(f)
